...
def load_model_dataset(args):
    ...
    if args.MODEL.split('_')[-1] =='C3D':
        model = torch.nn.DataParallel(models.C3D_SGA_STD(dropout_rate=args.dropout_rate,expand_k=args.expand_k,freeze_backbone=False,freeze_blocks=None).cuda().eval())
    else:
        model = torch.nn.DataParallel(models.I3D_SGA_STD(dropout_rate=args.dropout_rate,expand_k=args.expand_k,freeze_backbone=False,freeze_blocks=None).cuda().eval())

    if args.MODEL.split('_')[-1]=='C3D':
        if args.MODEL=='UCF_C3D':
            dataset=Test_Dataset_C3D(_C.TEST_H5_PATH,_C.TESTING_TXT_PATH,args.segment_len,args.ten_crop)
            model.load_state_dict(torch.load(_C.UCF_C3D_MODEL_PATH)['model'])
        else:
            dataset=Test_Dataset_SHT_C3D(_C.SHT_TEST_H5_PATH,_C.SHT_TEST_TXT_PATH,_C.SHT_TEST_MASK_DIR,
                                         args.segment_len,args.ten_crop)
            model.load_state_dict(torch.load(_C.SHT_C3D_MODEL_PATH)['model'])
        ...
        if args.MODEL=='UCF_I3D':
            ...
            model.load_state_dict(torch.load(_C.UCF_I3D_MODEL_PATH)['model'])
        else:
            dataset=Test_Dataset_SHT_I3D(_C.SHT_TEST_H5_PATH,_C.SHT_TEST_TXT_PATH,_C.SHT_TEST_MASK_DIR,
                                         args.segment_len,args.ten_crop)
            model.load_state_dict(torch.load(_C.SHT_I3D_MODEL_PATH)['model'])
...
def eval_UCF(args,model,test_dataloader):
    ...
    if args.vis:
        gradcam=GradCAM(model.module.Regressor,grad_pp=False)
